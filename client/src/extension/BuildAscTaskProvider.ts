...
export class BuildAscTaskProvider implements vscode.TaskProvider {
	...
	private getTask(version: string, definition?: BuildAscTaskDefinition): vscode.Task | undefined {
		...
		let execution
		if (extname === '.py') {
			execution = new vscode.ShellExecution(`docker run -t --rm --name build-pyteal -v "${this.workspaceRoot}:/project" -w /project obsidians/pyteal:0.5.4 /bin/bash -c "python ${file} > ${file.replace('.py', '.teal')}"`)
